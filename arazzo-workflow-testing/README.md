...
12. Set the first `userEmail` as `blr@specmatic.io` and the second `userEmail` as `del@specmatic.io`. Click on `Save` to save the input data.
...
14. You should see 11 successful assertions in the workflow test results, indicating that the workflow executed successfully.
15. You can click on the `Flow Chart` button to visualize the workflow execution and see the interactions between the services.
