...
## Learner task
Fix the typo in `specs/service.yaml` by changing `GET /pets/searchs` to `GET /pets/search`.
...
- The contract test for `GET /pets/searchs` fails because the application does not implement that route.
- Specmatic fetches the app's dynamically generated OpenAPI document from `/openapi.json`.
- Coverage report shows `/pets/search` as `Missing In Spec`.
- Coverage report also shows `/pets/searchs` as `Not Implemented`.
...
```terminaloutput
50%      /pets/searchs  GET   200   1   not implemented
         /pets/searchs  GET   422   0   missing in spec
0%       /pets/search   GET   0     0   missing in spec
```
...
Total missed endpoints count: 1 is greater than the maximum threshold of 0.
```
...
- Open `build/reports/specmatic/html/index.html` in your browser.
- Review the coverage mismatch there in a more readable report format.
- This is the dedicated API coverage report generated by Specmatic.
- The console summary is still the most precise place to verify every row in the baseline mismatch.
...
## Verify generated coverage artifacts
After a run, Specmatic also generates report artifacts inside this lab folder.

Inspect:

```shell
ls build/reports/specmatic
```

Important file:
- `build/reports/specmatic/coverage_report.json`
...
- Studio tests only the checked-in `service.yaml`, so it runs the typoed `/pets/searchs` scenario from the checked-in spec.
- The FastAPI Swagger UI shows that the app also publishes `GET /pets/search`.
- That mismatch is why the CLI coverage report marks `/pets/search` as `Missing In Spec` and `/pets/searchs` as `Not Implemented`.
...
- Coverage report shows `/pets/search` as `Missing In Spec`.
- Coverage report shows `/pets/searchs` as `Not Implemented`.
- Coverage report also shows the unexpected `422` response on `/pets/searchs` as `Missing In Spec`.
...
- If you do not see `/pets/search` in Swagger UI, confirm the provider is running on `127.0.0.1:8080`.
- If the first run does not fail as expected, confirm the checked-in spec still says `/pets/searchs` and not `/pets/search`.
...
- With `swaggerUrl`, Specmatic can fetch the app's published OpenAPI document and identify implemented routes missing from the checked-in spec.
- Current documentation emphasizes actuator-based discovery, but the current implementation also supports Swagger/OpenAPI fetch for this purpose.
