...
- A JSON response is returned for pet ID `1`.

Try loading pet ID `1` multiple times. Notice that the response is always the same.

Why does this happen? Open `specs/service.yaml` and look for the inline example named `SCOOBY_200_OK`. Because the contract already defines an example for pet ID `1`, the mock uses that example and returns the same response every time.

Try additional IDs:
- `2` -> success with contract-compliant dynamic data generated by Specmatic Mock. Notice that each time you try the same ID, you get a different response. This shows that the mock is generating data from the contract rather than just returning a static example.
...
3. Click **Generate** for `GET /pets/{petid}` with status `200`.
4. Open the generated example and note the generated `petId` and response body.
5. Go to the **Mock** tab.
6. Set mock port to `9100`.
7. Click **Run**.

Now go back to the consumer UI:
1. Enter the same generated `petId`.
2. Click **Load Pet** multiple times.

Expected output:
- Status shows `Success`.
- The JSON response matches the example you generated in Studio.
- Repeating the request with the same generated `petId` returns the same response, because the Studio mock now uses the generated externalized example instead of returning fresh dynamic data for that request.

To inspect mock traffic in Studio:
1. In **Mock** tab, open `GET /pets/{petid}` result.
2. Open `Scenario: GET /pets/(petid:number) -> 200`.
3. Review request and response details and confirm they match the generated example.

## Pass criteria
- Without mock: consumer shows `Service unavailable`.
- With mock running: consumer shows `Success` and returns JSON.
- With Studio-generated example and the same `petId`: repeated requests return the same JSON response.
...
- Specmatic mock behavior is generated from the contract (Industry standard specifications), so consumer expectations and the contract stay aligned.
