...
```shell
docker compose run --rm --no-deps --entrypoint sh api-test -lc "sed -i 's#[$]match(exact: approved)#\$match(pattern: approved\|verified)#' examples/test_finance_user_11.json"
```
...
```shell
docker compose run --rm --no-deps --entrypoint sh api-test -lc "sed -i 's#[$]match(exact: VRF-123456)#\$match(pattern: VRF-[0-9]{6})#; s#[$]match(exact: 2026-03-17)#\$match(dataType: date)#' examples/test_support_user_55.json"
```
